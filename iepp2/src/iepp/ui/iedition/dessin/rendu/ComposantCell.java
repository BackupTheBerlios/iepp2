...
public class ComposantCell extends IeppCell {
	...
	protected boolean contientProdMasquer;
	...
	public ComposantCell(IdObjetModele comp, int x, int y) { 
		...
		i = new ImageIcon(cheminImageComposantIepp + imageComposant);
	...
	public ComposantCell(IdObjetModele comp, int x, int y, int l, int h, String imagecomp) {
		super(((ComposantProcessus)comp.getRef()).getNomComposant());
		...
		// Définition de l'image du composant
		File fimg = new File(cheminImageComposantIepp+ imagecomp);
		
		if(fimg.exists()){
			i = new ImageIcon(cheminImageComposantIepp+ imagecomp);
			this.imageComposant = imagecomp;
		}else{
			fimg = new File(cheminImageComposantUser+ imagecomp);
			
			if(fimg.exists()){
				i = new ImageIcon(cheminImageComposantUser+ imagecomp);
				this.imageComposant = imagecomp;
			}else{
				i = new ImageIcon(cheminImageComposantIepp+ refImageComposant);
				this.imageComposant = refImageComposant;
			}
		}
	...
	public void setImageComposant(String imageComposant) {
		...
		// Définition de l'image du composant
		File fimg = new File(cheminImageComposantIepp+ imageComposant);
		
		if(fimg.exists()){
			i = new ImageIcon(cheminImageComposantIepp+ imageComposant);
			this.imageComposant = imageComposant;
		}else{
			fimg = new File(cheminImageComposantUser+ imageComposant);
			
			if(fimg.exists()){
				i = new ImageIcon(cheminImageComposantUser+ imageComposant);
				this.imageComposant = imageComposant;
			}else{
				i = new ImageIcon(cheminImageComposantIepp+ refImageComposant);
				this.imageComposant = refImageComposant;
	...
	public void setCompProc(ComposantProcessus cp) {
		this.compProc = cp;
	}
	...
	public void setContientProdMasquer(){
		this.contientProdMasquer = true;
	}
	
	public void setEnleverProdMasquer(){
		this.contientProdMasquer = false;
	}
	
	public boolean isContientProdMasquer(){
		return contientProdMasquer;
	}
