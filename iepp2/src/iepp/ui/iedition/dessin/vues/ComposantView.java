...
public class ComposantView extends VertexView {
	...
	public Point getPerimeterPoint(Point source, Point p) 
	{
		...
		File fimg = new File(IeppCell.cheminImageComposantIepp + ((IeppCell)getCell()).getImageComposant());
		
		if(fimg.exists()){
			i = new ImageIcon(IeppCell.cheminImageComposantIepp + ((IeppCell)getCell()).getImageComposant());
		}else{
			fimg = new File(IeppCell.cheminImageComposantUser+ ((IeppCell)getCell()).getImageComposant());
			if(fimg.exists()){
				i = new ImageIcon(IeppCell.cheminImageComposantUser+ ((IeppCell)getCell()).getImageComposant());
			}else{
				i = new ImageIcon(IeppCell.cheminImageComposantIepp + IeppCell.refImageComposant);
			}
		}
